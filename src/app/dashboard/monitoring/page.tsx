...
export default async function MonitoringPage({
  ...
  if (!isUmkmUser && !activeUmkmId) {
    ...
    return (
      <SelectUmkmClient
        ...
        targetLabel="perkembangan usaha"
        user={user}
      />
    );
  }

  // Fetch list of UMKM in parallel only if needed, along with monitoring records
  ...
  const [umkmResult, monitoringListResult] = await Promise.all([
    umkmListPromise,
    supabaseAdmin
      .from("monitoring")
  ...
  const monitoringListRaw = monitoringListResult.data;

  const monitoringList = monitoringListRaw?.map(m => ({
    ...m,
    nama_umkm: m.umkm?.nama_umkm
  })) || [];

  return <MonitoringClient monitoringList={monitoringList} umkmList={umkmList || []} user={user} activeUmkmId={activeUmkmId!} />;
}

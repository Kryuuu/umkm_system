...
export default async function ProdukPage({
  ...
  // Fetch list of UMKM in parallel only if needed, along with products list
  ...
  const [umkmResult, produkListResult] = await Promise.all([
    umkmListPromise,
    supabaseAdmin
      .from("produk")
  ...
  const produkListRaw = produkListResult.data;

  const produkList = produkListRaw?.map(p => ({
    ...p,
    nama_umkm: p.umkm?.nama_umkm
  })) || [];

  return <ProdukClient produkList={produkList} umkmList={umkmList || []} user={user} activeUmkmId={activeUmkmId!} />;
}
